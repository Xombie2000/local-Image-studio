...
struct StudioStateTests {
    @MainActor static func main() throws {
        ...
        precondition(HelperPresentation.name(for: "ollama::qwen3:8b") == "Ollama — Qwen3:8B")
        ...
        store.chooseHelper("omlx::server/exact-chat-id")
        store.promptHelper = PromptHelperAvailability(models: ["omlx::server/exact-chat-id"], available: true, model: "omlx::server/exact-chat-id", notice: nil)
        store.restoreModelPreferences()
        precondition(store.helperModelID == "omlx::server/exact-chat-id" && !store.helperUnavailable)
        store.promptHelper = PromptHelperAvailability(models: ["lms::server/exact-chat-id"], available: true, model: "lms::server/exact-chat-id", notice: nil)
        store.restoreModelPreferences()
        precondition(store.helperModelID == "omlx::server/exact-chat-id" && store.helperUnavailable)
        store.chooseImageModel("krea2_turbo")
