# Patch: mflux/models/seedvr2/model/seedvr2_transformer/attention.py
# Source: mflux 0.19.1 + MLX 0.32.2
# Purpose: SeedVR2 windowed attention with spatial partitioning and 3D RoPE
# Compatibility: Works with MLX 0.32.2 mx.fast.scaled_dot_product_attention
#
# Key features:
#   - MMAttention: Multi-modal attention (video + text)
#   - WindowPartitioner: Spatial window partitioning for memory efficiency
#   - RoPEModule: 3D rotary position embeddings
#   - Uses mx.fast.scaled_dot_product_attention(q, k, v, scale=...)
#
---
...
class MMAttention(nn.Module):
    ...
    def __call__(self, vid, txt, vid_shape, txt_shape):
        ...
        if self.rope_on_text:
            q_vid, k_vid, q_txt_rep, k_txt_rep = self.rope(
                ...
                txt_q=q_txt_rep,
                txt_k=k_txt_rep,
                txt_shape=MMAttention._repeat_with_counts(txt_shape, counts, axis=0),
            )
        else:
        ...
        win_lens = vid_lens + txt_len[MMAttention._repeat_indices(len(counts), counts)]
    ...
    def _repeat_text_for_windows(txt, txt_len, counts):
        ...
        return MMAttention._repeat_with_counts(txt, counts, axis=0).reshape(-1, *txt.shape[2:])

    @staticmethod
    def _repeat_with_counts(txt, counts, axis=0):
        """Repeat rows of txt according to per-row counts (MLX-compatible)."""
        import mlx.core as mx
        indices = mx.concatenate([mx.full((int(c),), i, dtype=mx.int32) for i, c in enumerate(counts)])
        return txt[indices]

    @staticmethod
    def _repeat_indices(n, counts):
        """Generate repeat indices for arange(n) according to per-element counts."""
        import mlx.core as mx
        return mx.concatenate([mx.full((int(c),), i, dtype=mx.int32) for i, c in enumerate(counts)])
    ...
    def _unconcat_and_coalesce(combined, vid_lens, txt_len, counts):
        win_to_batch = MMAttention._repeat_indices(len(txt_len), counts)
